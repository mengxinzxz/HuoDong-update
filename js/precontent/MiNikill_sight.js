...
export default function () {
    ...
    const onclickInterface = function () {
        ...
        //创建导航项
        let currentActiveItem = null, str = `${get.plainText(lib.translate['MiNikill_character_config'])}·`;
        menuItems.forEach((link, index) => {
            const navItem = document.createElement('button');
            navItem.className = 'nav-item';
            navItem.link = link;
            navItem.innerHTML = lib.translate[link];
            if (navItem.innerHTML.startsWith(str)) navItem.innerHTML = navItem.innerHTML.slice(str.length);
