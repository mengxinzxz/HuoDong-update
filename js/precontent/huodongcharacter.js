...
const packs = function () {
    const huodongcharacter = {
        ...
        characterIntro: {
            ...
            bilibili_xiaoyaoruyun: '逍遥如云，肘击群内抽象人士之一<span style="background-color: #000; color: #000; padding: 0 2px; border-radius: 2px; cursor: pointer; text-decoration: line-through; transition: all 0.3s ease;" onmouseover="this.style.backgroundColor=\'transparent\'; this.style.color=\'inherit\';" onmouseout="this.style.backgroundColor=\'#000\'; this.style.color=\'#000\';">（孩子们，其实是牢萌干的，我有十分甚至九分确信是他害的）</span>，因在各大<span style="background-color: #000; color: #000; padding: 0 2px; border-radius: 2px; cursor: pointer; transition: all 0.3s ease;" onmouseover="this.style.backgroundColor=\'transparent\'; this.style.color=\'inherit\';" onmouseout="this.style.backgroundColor=\'#000\'; this.style.color=\'#000\';">（其实仅一小部分）</span>无名杀群聊内大肆<span style="background-color: #000; color: #000; padding: 0 2px; border-radius: 2px; cursor: pointer; transition: all 0.3s ease;" onmouseover="this.style.backgroundColor=\'transparent\'; this.style.color=\'inherit\';" onmouseout="this.style.backgroundColor=\'#000\'; this.style.color=\'#000\';">（其实规模并没有那么大）</span>传播病毒武将扩展<span style="background-color: #000; color: #000; padding: 0 2px; border-radius: 2px; cursor: pointer; transition: all 0.3s ease;" onmouseover="this.style.backgroundColor=\'transparent\'; this.style.color=\'inherit\';" onmouseout="this.style.backgroundColor=\'#000\'; this.style.color=\'#000\';">（其实真的有病毒，还有木马和蠕虫）</span>，从而变得<span style="background-color: #000; color: #000; padding: 0 2px; border-radius: 2px; cursor: pointer; transition: all 0.3s ease;" onmouseover="this.style.backgroundColor=\'transparent\'; this.style.color=\'inherit\';" onmouseout="this.style.backgroundColor=\'#000\'; this.style.color=\'#000\';">（其实并不怎么）</span>出名，至今仍在各大无名杀群聊里逍遥法外<span style="background-color: #000; color: #000; padding: 0 2px; border-radius: 2px; cursor: pointer; transition: all 0.3s ease;" onmouseover="this.style.backgroundColor=\'transparent\'; this.style.color=\'inherit\';" onmouseout="this.style.backgroundColor=\'#000\'; this.style.color=\'#000\';">（迫真）</span>。',
        ...
        skill: {
            ...
            bilibili_zhaxiang: {
                ...
                async cost(event, trigger, player) {
                    event.result = await player.chooseToDiscard('h', get.prompt2(event.skill), 'allowChooseAll', 'chooseonly', [1, Infinity]).set('ai', card => lib.skill.zhiheng.check(card)).forResult();
                },
                async content(event, trigger, player) {
            ...
            bilibili_meihua: {
                ...
                trigger: { global: 'gameStart', player: ['disableEquipBefore', 'chooseToDisableBefore'] },
                filter(event, player) {
                ...
                locked: false,
                content() {
                    if (trigger.name == 'disableEquip' || trigger.name == 'chooseToDisable') return;
                ...
                intro: {
                    mark(dialog, storage, player) {
                        dialog.addAuto(player.getCards('s', card => card.hasGaintag('bilibili_meihua')));
                    },
                    markcount(storage, player) {
                        return player.countCards('s', card => card.hasGaintag('bilibili_meihua'));
                    },
                    onunmark(storage, player) {
                        const cards = player.getCards('s', card => card.hasGaintag('bilibili_meihua'));
                        if (cards.length) {
                ...
                subSkill: {
                    unmark: {
                        ...
                        filter(event, player) {
                            ...
                            return !player.hasCard(card => card.hasGaintag('bilibili_meihua'), 's');
                        },
                        silent: true,
                        content() {
                    ...
                    global: {
                        enable: 'phaseUse',
                        filter(event, player) {
                            if (!lib.inpile.some(name => get.type(name) === 'equip')) return false;
                            return game.hasPlayer(current => current !== player && current.hasSkill('bilibili_meihua')) && player.countCards('he') && player.countCards('h');
                        },
                        ...
                        async content(event, trigger, player) {
                            ...
                            if (result.bool) await player.equip(get.autoViewAs({ name: result.links[0][2] }, [cards[1]]));
                        },
                        ai: {
                            order: 7,
                            expose: 0,
                            result: {
                                player(player) {
                                    ...
                                    const target = game.findPlayer(current => current !== player && current.hasSkill('bilibili_meihua'));
            ...
            bilibili_laosaozhipao: {
                ...
                subSkill: {
                    die: {
                        ...
                        onremove(player) {
                            ui.arena.setNumber(Number(ui.arena.dataset.number) - 1);
                            for (const i of [...game.players, ...game.dead].unique()) {
                                if (i === player) continue;
                                if (Number(i.dataset.position) >= Number(player.dataset.position)) i.dataset.position = (Number(i.dataset.position) - 1).toString();
                            }
                            player.out();
            ...
            bilibili_waifa_rewrite: {
                inherit: 'bilibili_waifa',
                async cost(event, trigger, player) {
                ...
                async content(event, trigger, player) {
                    ...
                    for (const type of ['basic', 'trick', 'equip']) {
                        ...
                        const num = event.cards.filter(i => get.type2(i, player) !== type).length;
                        if (num > 0) {
                            player.addTempSkill(`bilibili_waifa_${type}`);
                            player.addMark(`bilibili_waifa_${type}`, num, false);
                        }
                    }
                },
            },
        ...
        translate: {
            ...
            bilibili_waifa_rewrite_info: '出牌阶段开始时，你可以弃置三张牌并摸等量张牌。若你弃置的类别包含：基本牌，本回合你使用的前X张基本牌无距离和次数限制（X为其中的非基本牌数）；锦囊牌，本回合你使用的前Y张锦囊牌可以额外增加和减少至多Y名目标（Y为其中的非锦囊牌数）；装备牌，本回合你使用的前Z张装备牌时摸Z张牌（Z为其中的非装备牌数）。',
